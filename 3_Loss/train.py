...
parser.add_argument('--batch-class-size', default = 30, type = int,
    dest = 'num_identities',
    help = 'Number of classes per batch.'
)
parser.add_argument('--batch-class-image-size', default = 3, type = int,
    dest = 'num_instances',
    help = 'Number of images for each class per batch.'
)
parser.add_argument('--training-batch-size', default = 90, type = int,
    dest = 'sz_tr_batch',
    help = 'Number of samples per batch for training.'
)
parser.add_argument('--eval-batch-size', default = 32, type = int,
    dest = 'sz_ev_batch',
    help = 'Number of samples per batch for evaluation.'
)
...
tr_Dataset = dataset.CUB_Dataset(
...
dl_tr = torch.utils.data.DataLoader(
    dataset=tr_Dataset,
    batch_size = args.sz_tr_batch,
    shuffle = False,
    sampler=dataset.RandomIdentitySampler(dataset=tr_Dataset,
...
ev_Dataset = dataset.CUB_Dataset(
        root = config['dataset'][args.dataset]['root'],
...
criterion = proxynca.ProxyNCAUnstable(
